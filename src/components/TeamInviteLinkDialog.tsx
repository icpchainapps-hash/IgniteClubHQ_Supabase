...
import { Link2, Copy, Check, Loader2, Download, Share2, QrCode, Sparkles, UserPlus } from "lucide-react";
...
const roleOptions: { value: TeamRole; label: string; color: string }[] = [
  { value: "player", label: "Player", color: "bg-amber-500/20 text-amber-600 border-amber-500/30" },
  { value: "parent", label: "Parent", color: "bg-pink-500/20 text-pink-600 border-pink-500/30" },
  { value: "coach", label: "Coach", color: "bg-emerald-500/20 text-emerald-600 border-emerald-500/30" },
  { value: "team_admin", label: "Team Admin", color: "bg-blue-500/20 text-blue-600 border-blue-500/30" },
];
...
export default function TeamInviteLinkDialog({ teamId, teamName }: TeamInviteLinkDialogProps) {
  const { user } = useAuth();
  ...
  const getRoleLabel = (role: TeamRole) => {
    ...
    return option?.label || role.replace("_", " ");
  };
  ...
  const getQRCodeAsFile = async (): Promise<File | null> => {
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            const file = new File([blob], `${teamName}-invite-qr.png`, { type: 'image/png' });
            resolve(file);
          } else {
            resolve(null);
          }
  ...
  const createInviteMutation = useMutation({
    ...
    onSuccess: (token) => {
      const link = `https://igniteclubhq.app/join/${token}?install=true`;
            ...
            {!generatedLink ? (
              <>
                ...
                <div className="space-y-3">
                  <Label className="text-sm font-medium">Select Role for Invitees</Label>
                  ...
                  <p className="text-sm text-muted-foreground bg-muted/50 rounded-lg p-3">
                    Anyone with this link can join as <span className="font-medium">{selectedRoleOption?.label}</span>. The link can be used by multiple people.
                  </p>
                </div>
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className={selectedRoleOption?.color}>
                  ...
                  <Button 
                    ...
                    onClick={async () => {
                      ...
                      if (qrFile) {
                        ...
                        downloadLink.download = `${teamName}-invite-qr.png`;
